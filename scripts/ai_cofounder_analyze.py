...
def _existing_pending_titles(db) -> set[str]:
    """Get titles of existing suggestions — dedup set.

    v6.54 fix: includes ALL statuses (pending, accepted, implemented, rejected).
    Pre-v6.54 bug: only pending+accepted, so implemented suggestions could be
    re-generated by subsequent analyzer runs, cluttering the dashboard with
    duplicates of already-done work.

    Rejected suggestions are also included because the user has actively
    declined them — rerunning would re-propose work the user consciously
    rejected. If the underlying problem resurfaces later, the user can
    reopen/unreject manually.
    """
    rows = (
        db.query(AiSuggestion.title)
        .filter(AiSuggestion.status.in_(["pending", "accepted", "implemented", "rejected"]))
        .all()
    )
...
def cleanup_stale_pending(db) -> dict:
    """v6.54: auto-close pending suggestions that duplicate implemented ones.

    Se un pending ha stesso titolo di un implemented già esistente, lo
    marca come 'implemented' (considerato già risolto in passato).

    Ritorna dict con counts per audit.
    """
    from src.db.models import AiSuggestion
    from sqlalchemy import and_

    # Titoli implementati (già risolti in passato)
    impl_titles = {
        r[0]
        for r in db.query(AiSuggestion.title)
        .filter(AiSuggestion.status == "implemented").all()
    }

    # Pending con titolo che corrisponde a implemented
    stale_pending = db.query(AiSuggestion).filter(
        AiSuggestion.status == "pending",
        AiSuggestion.title.in_(impl_titles),
    ).all()

    for sug in stale_pending:
        sug.status = "implemented"
        sug.reviewed_at = datetime.now(timezone.utc).isoformat()
        sug.review_note = "Auto-closed: titolo gia' implementato in passato (v6.54 cleanup)"

    db.commit()

    return {
        "stale_pending_auto_closed": len(stale_pending),
        "implemented_titles_total": len(impl_titles),
    }


def run_analysis(db=None, cleanup_first: bool = True) -> dict:
    """Run all analysis functions and return summary.

    Accepts an optional db session for testing; creates its own if None.

    v6.54: con cleanup_first=True (default), PRIMA pulisce i pending
    duplicati di implementati, POI dedupl'analisi nuova.
    """
    ...
    try:
        cleanup_stats = {}
        if cleanup_first:
            cleanup_stats = cleanup_stale_pending(db)
